...
- `-crawl-permute` reuse every discovered path under every discovered directory
  level. When the crawl finds `/admin/panel` and has also seen the levels `/`,
  `/api/` and `/shop/`, it additionally tries `/api/admin/panel` and
  `/shop/admin/panel`. Paths and levels are pooled globally and combined
  retroactively, so a path found late is still tried under levels seen earlier.
  Off by default because it multiplies requests; bounded by `-crawl-permute-max`.
- `-crawl-permute-max` max URLs generated by `-crawl-permute` (default `1000`,
  `0` for unlimited).
