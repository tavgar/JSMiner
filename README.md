...
determinable, whether message data reaches a dangerous sink, which messages were
generated by JSMiner's own probes, how many listeners appear to inspect
`event.origin` / `event.source`, and the listener registration locations when
available. Origin/source inspection is reported as **evidence, not proof** —
JSMiner never claims an
"origin validation bypass" it did not actually test for. It also flags
cross-origin sends of URL-derived data.

### Triage hints and URL evidence

Every DOM finding includes a conservative `triage` hint: `confirmed`,
`worth_reviewing`, `likely_benign`, or `informational`, plus a plain-language
reason. The hint explains the evidence; it does not replace severity or claim a
vulnerability. Only a controlled execution beacon receives `confirmed`.

URL/navigation flows also report the resolved scheme and destination origin,
whether the destination stayed same-origin, the URL component containing the
marker, the input form (absolute, relative, query-only, and so on), and whether
an executable scheme was observed. This makes ordinary same-origin query or
fragment propagation distinguishable from cross-origin link control without
printing an unbounded second copy of the URL.
...
```json
{
  ...
  "triage": {
    ...
    "reason": "controllable data reached a markup-capable context; execution was not confirmed"
  },
  "fingerprint": "…"
}
```
